...
loadConfig(location).then(response => {
	/* Deze pagina is het bord zoals het was. Staat er "layout": "tiles" in config.json, dan gaat het
	   verder op tiles.html: dezelfde bronnen, een andere indeling, en een eigen opmaak die met deze
	   geen regel deelt - vandaar een eigen pagina en geen schakelaar hier. Zonder die instelling
	   verandert er niets aan wat je gewend bent. */
	if (document.config && document.config.layout === 'tiles') {
		window.location.replace('tiles.html' + window.location.search);
		return;
	}
