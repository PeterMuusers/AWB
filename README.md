...
#### Sat24
https://www.sat24.com/
#### Two boards, one checkout
`index.html` is the board as it always was and is not touched by any of this. The tile layout described below lives on its own page, `tiles.html`, with `css/tiles.css` and `scripts/tiles.js`; put `"layout": "tiles"` in config.json and `index.html` sends you there, leave it out and nothing changes. A separate page rather than a switch in one, because the two share no line of styling and half a transition looks worse than either end of it. On the Raspberry Pi the kiosk opens `tiles.html` directly, so it does not even see the redirect.

Everything else is shared: the modules, the proxies, the config file. So the old board gets the fixes too — the bulletin tile filled, the User-Agent the KNMI insists on, and the rewritten forecast when a key is set.
...
#### Wind map (Windy)
When the wind is worth a picture, the map tile shows Windy's wind field for a while, the same way the jumpruns take it over. This has thresholds of its own, higher than the ones that colour the cells in the wind table, and deliberately so: a coloured cell means "have a look at this", which is not every time worth half a screen. A map appears for more than 30 kt at 1.000, 2.000 or 3.000 ft, and for more than 20 kt on the ground. The low band shows its strongest level — two levels that hardly differ say the same thing — and both can apply at once, in which case both come by, from the ground up. Only the air a canopy flies through: wind that hard higher up does colour its cell in the wind profile, where the number stands, but it does not take over the map. Above the map stands the same number that stands elsewhere on the board, with the altitude it belongs to, and nothing else: what it means for the jump is the business of the people running it.

Enabled by the `windy` block in config.json — leave the block out and nothing is embedded: `seconds` how long each altitude stays up (30 by default: Windy loads its wind data after the map, and on a Raspberry Pi a shorter turn can be over before the field is painted), `afterRuns` how many complete radar loops go by first (0 keeps it off altogether), `everySeconds` the shortest time between two turns (180 by default, so the wind map comes up at most once every three minutes — a radar loop is as long as the number of frames it has, so counting loops alone gives a different rhythm on a busy day than on a quiet one), `product` the model Windy draws (`ecmwf` by default), `zoom` how much of the country is in view, and `lowBand`/`lowLimit` and `groundLimit` for the thresholds above. The altitudes come from `upperwinds`, so a board with other altitudes gets the ones of its own that fall inside a band. It uses Windy's public embed, which needs no key. Windy has its own colours and its own furniture; the logo, the legend and the buttons fall outside the frame because the iframe is larger than the window it sits in, and how much is cut away is `--windy-crop-x` and `--windy-crop-y` in the stylesheet. The iframe only has an address while it is on screen: Windy keeps a WebGL map running as long as it is loaded, and that is exactly the time the Raspberry Pi needs for the radar.

#### Tomorrow (`/vooruitzicht`)
`/vooruitzicht` in Discord puts the weather of tomorrow where the bulletin normally is, and leaves it there until `/normaal` takes it off again. On the left a table with the same altitudes as the wind profile beside it, at four moments of the day (10, 12, 14 and 16 local), with the expected cloud base above it and the ground wind with its gusts below. On the right the run of the day: the cloud layers the model expects on an altitude scale, the freezing level through them, and under that the ground wind and the gusts from 09:00 until sunset — because before nine nobody jumps and after sunset nobody does either.

It all comes from one request to Open-Meteo, the same model as the wind profile (`knmi_seamless`: HARMONIE for the short range, ECMWF beyond it), and it is parsed by the same code — `aloft.parse()` — so the two cannot disagree. The table cells are drawn by `aloft.cell()` for the same reason: one place decides what a wind value looks like. Tomorrow is sharp, the day after would be a trend; that is why this shows one day and not three.

`/normaal` is the one button that clears everything temporary: the fine-weather demo, a demo scene and this. The tile header becomes "Vooruitzicht · zaterdag 19 september" while it is up, because "Low Level Forecast" is not what you are reading then.

#### Demo screens
`/demo` in Discord puts a fixed scene on the board for half a minute and then the board is itself again: hard ground wind, hard wind low or high, the wind limit in the table, gusts, the freezing level under exit altitude, three cloud layers, a closed deck, thunderstorms in the bulletin, and a jumprun (one, or a high and a low one). They are there to show what the board does when it matters, without having to wait for the weather to do it.

Unlike the fine-weather demo (`/summertime`, see `scripts/demo.js`) the page is not reloaded: loading takes about ten seconds, which is a third of the demo. Instead the running modules have their values overwritten and redraw themselves, and every scene hands back a function that undoes exactly what it did. A jumprun demo is computed by the same calculation as a real one, with the wind of the moment, so it is what it would really look like — only the agreement is made up.

While a scene runs, **DEMO** stands in the header. The fine-weather demo does not need that (a palm island on the radar speaks for itself) but "24 kt on the ground" is not recognisable as a joke, and somebody is reading that screen who is about to jump. The board asks `demo.php` every five seconds, so a scene starts within five seconds of the command and ends by itself; the marker lives in `/run`, so no demo survives a restart of the Pi.
